...
import { fetchShipment, fetchShipmentShap, fetchShipmentTimeline, fetchShipmentDice } from '../lib/api'
...
export default function ShipmentDetail() {
  ...
  const risk = shipment ? Math.round((shipment.risk ?? 0.87) * 100) : 87
  ...
  const checkpoints = timeline?.timeline ?? [
    { name: 'Mumbai Port (Origin)', status: 'completed', time: 'Departure: Oct 12, 04:30 AM' },
    { name: 'Dubai Transit Hub', status: 'delayed', time: 'Estimated Delay: 34 Hours', tags: ['DELAYED', 'WEATHER'], vessel: 'Ever Apex V.024' },
    { name: 'London Gateway (Destination)', status: 'pending', time: 'Projected: Oct 19, 11:00 PM' },
  ]
    ...
    <div className="p-8 space-y-6 animate-fade-in">
      ...
      <div className="space-y-1">
        <div className="flex items-center gap-2 text-[10px] font-bold text-gray-500 uppercase tracking-widest">
          <Link to="/" className="hover:text-white transition-colors">Shipments</Link>
          <ChevronRight className="w-3 h-3" />
          <span className="text-dash-accent">{shipId}</span>
        </div>
        <h2 className="text-3xl font-bold text-white tracking-tight">Shipment Detail: {shipId}</h2>
        <p className="text-gray-500 text-sm">Real-time predictive risk analysis and AI intervention recommendation</p>
      </div>
      ...
      <div className="glass-card rounded-3xl p-8 relative overflow-hidden flex flex-col justify-between min-h-[220px]">
        <div className="relative z-10">
          ...
          <div className="flex flex-col gap-1">
            <p className="text-gray-500 text-xs font-bold uppercase tracking-widest">Calculated High Risk</p>
            <h3 className="text-6xl font-black text-white tracking-tighter">
              Risk Score: {risk}% <span className="text-3xl font-medium text-gray-400">± 5%</span>
            </h3>
          </div>
            ...
            {checkpoints.map((cp, i) => (
              <div key={i} className="flex gap-4">
                <div className="flex flex-col items-center">
                  ...
                  >
                    {cp.status === 'completed' && <Check className="w-3 h-3 text-black" />}
                    {cp.status === 'delayed' && <AlertTriangle className="w-3 h-3 text-white" />}
                    {cp.status === 'pending' && <MapPin className="w-3 h-3 text-gray-500" />}
                  </div>
                  {i < checkpoints.length - 1 && (
                    <div
                      className={`w-0.5 h-12 ${
                        cp.status === 'completed' ? 'bg-dash-green/30' : 'border-l-2 border-dashed border-gray-700'
                      }`}
                    />
                  )}
                </div>
                <div className="pb-6">
                  <div className="flex items-center gap-2">
                    <p className={`text-sm font-bold ${cp.status === 'delayed' ? 'text-dash-risk' : cp.status === 'pending' ? 'text-gray-500' : 'text-white'}`}>
                      {cp.name}
                    </p>
                    {cp.tags && (
                      <div className="flex gap-1">
                        {cp.tags.map((tag) => (
                          ...
                          >
                            {tag === 'WEATHER' && <Cloud className="w-2 h-2 inline mr-0.5" />}
                            {tag}
                          </span>
                        ))}
                      </div>
                  ...
                  {cp.vessel && (
                    <p className="text-[10px] text-gray-400 mt-1 italic">Vessel: {cp.vessel}</p>
                  )}
                </div>
              </div>
            ))}
              ...
              return (
                <div key={sv.feature} className="space-y-3">
                  <div className="flex justify-between text-[11px] font-bold uppercase tracking-widest">
                    ...
                    <span className={isPositive ? 'text-dash-risk' : 'text-dash-green'}>
                      {isPositive ? '+' : ''}{sv.impact.toFixed(2)} Impact
                    </span>
                  </div>
                  <div className={`h-2.5 w-full bg-white/5 rounded-full overflow-hidden ${!isPositive ? 'flex justify-end' : ''}`}>
                    <div
      ...
      <div className="glass-card rounded-3xl p-8 border border-dash-accent/30 relative overflow-hidden">
        ...
        <div className="flex flex-col gap-8">
          <div className="flex items-center gap-5">
            ...
            <div>
              <h4 className="text-2xl font-bold text-white tracking-tight">Recommended Intervention</h4>
              <p className="text-dash-accent font-semibold text-sm">Assign Alternative Carrier (FedEx-2) • Priority Express</p>
            </div>
          </div>
          ...
          <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
            <div className="bg-white/5 border border-white/10 rounded-2xl p-6">
              <p className="text-gray-500 text-[10px] font-bold uppercase tracking-widest mb-2">Action Cost</p>
              <h5 className="text-3xl font-bold text-white tracking-tight">₹12,000</h5>
              <p className="text-[10px] text-gray-500 mt-1">Expedited Handling Fee</p>
            </div>
            <div className="bg-white/5 border border-white/10 rounded-2xl p-6">
              <p className="text-gray-500 text-[10px] font-bold uppercase tracking-widest mb-2">SLA Saved</p>
              <h5 className="text-3xl font-bold text-dash-green tracking-tight">₹85,000</h5>
              <p className="text-[10px] text-gray-500 mt-1">Potential Penalty Avoided</p>
            </div>
            <div className="bg-dash-accent/10 border border-dash-accent/20 rounded-2xl p-6">
              <p className="text-dash-accent text-[10px] font-bold uppercase tracking-widest mb-2">Net Saving</p>
              <h5 className="text-3xl font-bold text-white tracking-tight">₹73,000</h5>
              <p className="text-[10px] text-dash-accent/70 mt-1">Total Economic Gain</p>
            </div>
          </div>
